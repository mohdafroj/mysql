...
export class SaleComponent implements OnInit {
	...
	constructor(
		...
		this.route.queryParams.subscribe((params: Params) => {
			...
			this.category      = ( params.category ) ? params.category : 'perfumes';
    ...
    getFilterProducts(){
		...
		this.products.getStoreOfferProducts(prms).subscribe(
            res => {
                if(res.status){
	...
	selectGender(value:string){
		if( value != '' ){			
			...
			if(this.brandKey > 0){ param.brand = this.brandKey; }
