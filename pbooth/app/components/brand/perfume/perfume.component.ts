...
export class BrandPerfumesComponent implements OnInit {
	...
	constructor(
		...
		this.brandKey 		= ( Array.isArray(urls) && urls[1] && urls[1].path ) ? urls[1].path : '';
		...
		if ( this.currentPath === 'perfume-bottle' ) { // for perfume bottle
			...
			this.bannerImage = 'assets/images/banners/current/perfume.jpg';
			...
			this.bannerImage = 'assets/images/banners/current/deodorant.jpg';
			...
			this.bannerImage = 'assets/images/banners/current/body-mist.jpg';
		}
		...
		this.products.getFilterProducts(prms).subscribe(
            res => {
                if(res.status){
					...
					this.bannerShow  = res.data.bannerShow;
