...
class CouponComponent extends Component
{
    ...
    {
        $rulesData = [];
        $dataTable = TableRegistry::get('SubscriptionApi.CartRules');
        $query = $dataTable->find('all')
            ...
            ->toArray();
        $rulesData['status'] = 0;
        if ( PC['BANNER_STATUS'] == 1 ) {
            return ['status'=>0,'freeShip'=>'no','couponDiscount'=>0.0,'msg'=>'Coupon code is not applicable during sales!'];
        }
        if (empty($query)) {
            ...
            if (($query->status == 'active') && ($query->_matchingData['Coupons']->status == 'active')) {
                ...
                if (($crtDate > $toDate) && $toDate) {
                    ...
                    //$products = [['id'=>2,'price'=>200,'quantity'=>2,'category'=>5,'brand'=>4],['id'=>4,'price'=>200,'quantity'=>3,'category'=>5,'brand'=>4],['id'=>11,'price'=>200,'quantity'=>1,'category'=>5,'brand'=>4]];
                    $ids = [];
                    $qty = 0;
                    ...
                    //Check products for discount availability in your cart
                    if (count($cd['categories'])) {
                        $usedStatus = 1;
                        //if array is 2-D then sort by  
                        if ( isset($cd['categories'][0]['category']) ) {
                            $percentCategory = $rupeesCategory = [];
                            foreach ($cd['categories'] as $item) {
                                if ( $item['type'] == 'percentage' ) {
                                    array_push($percentCategory, $item); // insert into end
                                } else {
                                    array_push($rupeesCategory, $item); // insert into end
                                }
                            }
                            $percentCategory = $this->sortCategoryByDiscount($percentCategory);
                            $rupeesCategory = $this->sortCategoryByDiscount($rupeesCategory);
                            $cd['categories'] = array_merge($percentCategory, $rupeesCategory);
                        }
                        //pr($products);die;
                        foreach ($cd['categories'] as $category) {
                            ...
                            if( is_array($category) ){ 
                                ...
                                $type = $category['type'];
                                $discount = ( $category['discount'] > 0 ) ? $category['discount'] : $discountValue;
                            } else {
                                $categoryId = $category;
                            }
                            //pr($cd['categories']);
                            for ($i = 0; $i < count($products); $i++) {
                                ...
                                if( !isset($products[$i]['type']) && in_array($categoryId, $products[$i]['category']) && ( ($brandId == $products[$i]['brand']) || ($brandId == 0) ) ){
                                    $discountProducts[] = $products[$i]['id'];
                                    $usedStatus = 0;
                    ...
                    if (count($minDiscountProducts) > 0) {
                        $discountProducts = $minDiscountProducts;
                    } else {
                        $rulesData['status'] = 0;
                        $rulesData['msg'] = "Sorry, this coupon applicable on minimun price of product is Rs $minPrice";
                    }

                }
                //Final Calculation for discount
                $couponDiscount = $tempTotal = $tierStatus = 0;
                if ($rulesData['status']) {
                    $totalSelectedProducts = count($products);
                    for ($i=0; $i < $totalSelectedProducts; $i++) {
                        switch ( $discountType ) {
                            case 'tier1': $discount = 0; $tierStatus = 1; $products[$i]['tier'] = $products[$i]['tier1'] ?? $products[$i]['price'];
                                break;
                            case 'tier2': $discount = 0; $tierStatus = 1; $products[$i]['tier'] = $products[$i]['tier2'] ?? $products[$i]['price'];
                                break;
                            case 'tier3': $discount = 0; $tierStatus = 1; $products[$i]['tier'] = $products[$i]['tier3'] ?? $products[$i]['price'];
                                break;
                            default:
                                if (in_array($products[$i]['id'], $discountProducts)) {
                                    $type = $products[$i]['type'] ?? $discountType;
                                    $discount = $products[$i]['discount'] ?? $discountValue;
                                    if ($type == 'rupees') {
                                        $couponDiscount +=  $discount * $products[$i]['quantity'];
                                    }else{
                                        $couponDiscount += (float) ($products[$i]['price'] * $products[$i]['quantity'] * $discount) / 100;
                                    }
                                    $products[$i]['discount'] = $couponDiscount;
                                }
                        }
                    }
                    $rulesData['products'] = $products;
                    $rulesData['tierStatus'] = $tierStatus;
                    $rulesData['discountType'] = $type ?? $discountType;
                    $rulesData['discountValue'] = $discount ?? $discountValue;
                }
                $rulesData['couponDiscount'] = $couponDiscount;
            } else {
                $rulesData['msg'] = 'Sorry, coupon code is inactive!';
            }
        } //pr($rulesData['couponDiscount']);
        return $rulesData;
    }
    ...
    public function inOrderStatus($coupon, $email)
    {
        ...
        if (!empty($query)) {
            ...
            if ($dataTable->save($rule)) {
                $dataTable = TableRegistry::get('SubscriptionApi.Coupons');
            ...
            if (empty($query)) {
                ...
                if (($query->status == 'active') && ($query->_matchingData['Coupons']->status == 'active')) {
                    ...
                    if (($crtDate > $toDate) && $toDate) {
                        ...
                        if (count($cd['categories'])) {
                            $query = TableRegistry::get('SubscriptionApi.ProductCategories')->find('all', ['fields' => ['product_id', 'category_id'], 'conditions' => ['product_id IN' => $ids]])->distinct(['category_id'])->toArray();
                            if (!empty($query)) {
                                $usedStatus = 1;
                                foreach ($query as $v) {
    ...
    public function sortCategoryByDiscount($a) {
        $l = count($a);
        for ($i=0; $i < $l; $i++) {
            for ($j= $i + 1; $j < $l; $j++) {
                if ( $a[$i]['discount'] < $a[$j]['discount'] ) {
                    $temp = $a[$j];
                    $a[$j] = $a[$i];
                    $a[$i] = $temp;
                }
