...
class ShiproketComponent extends Component
{
	...
	public function createToken(){
		...
		try{
			$http 		= new Client();
			$response 	= $http->post($this->base_url.'auth/login', json_encode(['email'=>PC['TEST_EMAIL'],'password'=>'786afroj']), ['headers'=>['Content-Type'=>'application/json']]);
	...
	public function getCouriers(){
		$couriers = [];
		$couriers = TableRegistry::get('SubscriptionApi.Couriers')->find('all',[])->hydrate(false)->toArray();
		return $couriers;
	}
	...
	public function findCourier($title){
		$courier = [];
		if( !empty($title) ){
			$courier = TableRegistry::get('SubscriptionApi.Couriers')->find('all',['conditions'=>['title'=>$title]])->hydrate(false)->toArray();
		}
		$courier = $courier[0] ?? [];
		return $courier;
	}
	
	public function checkPincode($countryCode2, $weight = 0.5){
		$data 		= [];
		$status 	= 0;
		$message 	= 'Sorry, service not available!';
		$checkPincode = TableRegistry::get('SubscriptionApi.Systems')->checkInvalidPincodes($pincode);
		if ( $checkPincode !== 1 ) {
			if( !empty($countryCode2) ){
				$http 		= new Client();
				$this->token = $this->getToken();
				$params		= ['cod'=>0,'weight'=>$weight,'pickup_postcode'=>110015,'delivery_country'=>$countryCode2];
				$response 	= $http->get($this->base_url.'courier/international/serviceability/', $params, ['headers'=>['Authorization'=>$this->token, 'Content-Type'=>'application/json']]);
				$response 	= $response->json;
	...
	public function getCountryCodeByPincode($pincode){
		$http 		= new Client();
		$response 	= $http->get('http://ziptasticapi.com/'.$pincode, [], ['headers'=>['Content-Type'=>'application/json']]);
		$response 	= $response->json;
	...
	public function sendOrder($orderId, $courierId){
		...
		try{
			...
			->contain(['OrderDetails'=>function($q){
				return $q->select(['order_id','product_id','title','sku_code','quantity','price','size','discount']);
			}])
			...
			if( isset($saveRes['status']) && ($saveRes['status'] == 1) ) {
				...
				$orderUpdate->push_response = json_encode($saveRes);
				if( $orderTable->save($orderUpdate) ){
					...
					if( $invoiceId > 0 ){
						...
						$invoiceUpdate->courier_id 			= $courierId;
	...
	public function syncStatus(){
		try{
			$shippingLogTable	= TableRegistry::get('SubscriptionApi.OrderShippingLogs');
			$orderTable 		= TableRegistry::get('SubscriptionApi.Orders');
				...
				switch( $newStatus ){
					case 7: //'for Delivered': 
						...
						if( $order->status != 'delivered' ){
							...
							$this->Store->orderStatusEmails($id, 'delivered');
						}
						break;
