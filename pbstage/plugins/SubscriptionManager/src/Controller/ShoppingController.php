...
class ShoppingController extends AppController
{
	...
	public function index()
    {
		...
		if( !empty($status) ) { $filterData['CartRules.status'] = $status; }
    ...
    public function addRule($key=null, $md5=null)
    {
        ...
        if ($this->request->is('post')) {
			...
			$validator
				...
				->inList('discount_type', ['percentage','rupees','tier1','tier2','tier3']);
		...
		$cateTree = TableRegistry::get('SubscriptionManager.Categories')->find('threaded', ['order'=>'Categories.lft'])->where(['is_active' => 'active'])->toArray();
        $brands = TableRegistry::get('SubscriptionManager.Brands')->find('all', ['fields' => ['id', 'title'], 'order' => ['title' => 'asc']])->where(['is_active' => 'active'])->toArray();
    ...
    public function addCoupons($id=null, $key=null, $md5=null)
    {
		...
		$coupons = TableRegistry::get('SubscriptionManager.Coupons')->find('all', ['conditions'=>$filterData]);
		if( !empty($createdFrom) && !empty($createdTo) ){
