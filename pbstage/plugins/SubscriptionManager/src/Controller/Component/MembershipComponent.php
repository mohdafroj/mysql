...
use SubscriptionManager\Controller\Component\Store;
...
class MembershipComponent extends Component
{
	...
	public function add($data){
		...
		if( $customerId > 0 ){
			...
			$dataTable 			= TableRegistry::get('SubscriptionManager.Memberships');
	...
	public function walletDataPending($customerId, $orderId){
		...
		if( $status ){
			...
			$dataTable = TableRegistry::get('SubscriptionManager.OrderDetails');
			$query = $dataTable->find();
