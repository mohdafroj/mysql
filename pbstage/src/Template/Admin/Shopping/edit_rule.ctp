<?php
echo $this->Element('Admin/Shopping/top_menu');
$usages = $customData['usages'] ?? 0;
$free_ship = $customData['free_ship'] ?? 'no';
$min_qty = $customData['min_qty'] ?? 1;
$min_price = $customData['min_price'] ?? 1;
$categoriesIds = $customData['categories'] ?? [];

$usedCustomers = $customData['used_customers'] ?? [];
$validCustomers = $customData['valid_customers'] ?? [];

$emails = '';
foreach ($validCustomers as $value) {
    $emails = empty($emails) ? $value : $emails . ',' . $value;
}
?>
                                ...
                                <div class="box-body">
										...
										<div class="col-sm-9">
											<div class="col-sm-6 col-xs-12 no-padding-left xs-no-padding">
												<div class="input-group date">
													...
													<?=$this->Form->select('discount_type', $this->Admin->discountType, ['style' => 'width:100%;', 'class' => 'form-control'])?>
												</div>
												<div class="text-red">
										...
										<div class="col-sm-9">
											<div class="col-sm-6 col-xs-12 no-padding-left xs-no-padding">
												<div class="input-group date">
													...
													<?=$this->Form->text('usages', ['value' => $usages, 'class' => 'form-control', 'placeholder' => 'Enter number for usages times!']);?>
													<div class="input-group-addon">Times</div>
												</div>
												<div class="text-red">
													<?php echo isset($error['usages']['integer']) ? $error['usages']['integer'] : null; ?>
												</div>
											</div>
											<div class="col-sm-6 col-xs-12 no-padding-right xs-no-padding">
												<div class="input-group date">
													<div class="input-group-addon">Status <span class="text-red">*</span></div>
													<?=$this->Form->select('status', $this->Admin->couponStatus, ['empty' => false, 'style' => 'width:100%;', 'class' => 'form-control'])?>
												</div>
												<div class="text-red">
										...
										<div class="col-sm-9">
											<div class="col-sm-6 col-xs-12 no-padding-left xs-no-padding">
												<div class="input-group date">
													...
													<?=$this->Form->select('free_ship', ['no' => 'No', 'yes' => 'Yes'], ['value' => $free_ship, 'empty' => false, 'style' => 'width:100%;', 'class' => 'form-control'])?>
												</div>
											</div>
											<div class="col-sm-6 col-xs-12 no-padding-right xs-no-padding">
												<div class="input-group date">
													<div class="input-group-addon">Mini Qty for discount to applied</div>
													<?=$this->Form->text('min_qty', ['value' => $min_qty, 'class' => 'form-control', 'style' => 'width:100%;']);?>
												</div>
												<div class="text-red">
													<?php echo $error['min_qty']['integer'] ?? null; ?>
												</div>
											</div>
										</div>
									</div>
									<div class="form-group">
										...
										<div class="col-sm-9">
											<div class="col-sm-6 col-xs-12 no-padding-left xs-no-padding">
												<div class="input-group date">
													<div class="input-group-addon">Minimum Amount of Product(<?php echo $this->Admin->priceLogo; ?>)<span class="text-red">*</span></div>
													<?=$this->Form->text('min_price', ['value' => $min_price, 'class' => 'form-control', 'style' => 'width:100%;']);?>
												</div>
												<div class="text-red">
...
function cateFun($cateTreeItem, $categories, $data = '')
{
    if (count($cateTreeItem)) {
        foreach ($cateTreeItem as $chilName) {
            ...
            if ($child > 0) {
                //echo '<li class="treeview active"><label><input type="checkbox" name="chk[]" value="' . $chilName->id . '" class="minimal" ' . $checked . ' /></label><a href="">' . $chilName->name . ' (' . $child . ')<span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a><ul class="treeview-menu menu-open">';
                echo '<li class="treeview active"><label style="width:90%;">' . $chilName->name . ' (' . $child . ')</label>
...
?>
												<ul class="sidebar-menu"><?php cateFun($cateTree, $categoriesIds);?></ul>
										...
										<div class="col-sm-4">
											<div class="form-group">
												...
												<?=$this->Form->textarea('emails', ['value' => $emails, 'class' => 'form-control', 'rows' => 10, 'style' => 'width:100%;', 'placeholder' => 'Please enter valid email id!']);?>
											</div>
										</div>
									</div>
									<div class="form-group">
								...
								<?php if (count($usedCustomers)) {?>
											<strong>This Rule used by customers, so don't edit more! </strong>
								<?php }?>
										</div>
									</div>
                                </div>
...
<style>
	...
	.skin-blue .tree_div .sidebar-menu > li > .treeview-menu{padding-left:15px;}
	.tree_div ul.sidebar-menu li a{display:inline-block; padding-left:5px; width:90%;}
</style>

<script>
	var brands = <?php echo json_encode($brands); ?>;
	var selectedCatBrand = <?php echo json_encode($categoriesIds); ?>;
	...
	function setCategoryBrands(arr){
		...
		$.each(arr,function(key, value){
			...
			if( value.category ) {
				...
				selectedArr.push({"category":value,"brand":0,"type":$("[name='discount_type']").children("option:selected").val(), "discount":$("[name='discount_value']").val()});
			}
		});
		selectedCatBrand = selectedArr;
		$("[name='categories']").val(JSON.stringify(selectedCatBrand));
		return false;
	}
