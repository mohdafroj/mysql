        ...
        <div class="col-sm-12 col-xs-12 table_view responsive-mobile-table"><!-- start of table -->
            <table class="col-xs-12 table-bordered table-hover table-condensed no-padding no-border">
                <thead>
                    <tr>
                        ...
                        <th><?= $this->Paginator->sort('valid_from', 'From Date') ?></th>
                        <th><?= $this->Paginator->sort('valid_to', 'To Date') ?></th>
                ...
                <tbody>                    
                    <tr><!-- start of row_1 -->
                        ...
                        <td data-title="Rule Title">                            
                        	<?= $this->Form->text('title', ['value'=>$title, 'class'=>'form-control', 'placeholder'=>'Search by title']); ?>
                        </td>
                        <td data-title="Coupon Code">
                        	<?= $this->Form->text('coupon', ['value'=>$coupon, 'class'=>'form-control', 'placeholder'=>'Search by coupon code']); ?>
                        </td>
                        <td data-title="Discount Value">
                            <div class="input-group date">
                                <?= $this->Form->select('discount_type', $this->Admin->discountType, ['style'=>'width:50%;','class'=>'form-control'])?>
                        ...
                        <td data-title="From Date">
                        	<div class="input-group date">
                        		...
                        		<?= $this->Form->text('valid_from', ['value'=>$validFrom, 'id'=>'datepicker1', 'class'=>'form-control', 'placeholder'=>'Search by from date']); ?>
                        	</div>
                        </td>
                        <td data-title="To Date">
                        	<div class="input-group date">
                        		<div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                        		<?= $this->Form->text('valid_to', ['value'=>$validTo, 'id'=>'datepicker2', 'class'=>'form-control', 'placeholder'=>'Search by to date']); ?>
                        	</div>
                        </td>
                        <td data-title="Status">
                            <?= $this->Form->select('status', $this->Admin->couponStatus, ['value'=>$status,'style'=>'width:100%;','class'=>'form-control'])?>
                        </td>
           ...
           <?php foreach ($cartRule as $value):?>         
                    <tr>
                        ...
                        <td data-title="Rule Title"><?= h($this->Admin->checkValue($value->title)) ?></td>
                        <td data-title="Coupon Code">
                            <?php
                                if( isset($value->coupons[0]->coupon) ){
                                    echo h($this->Admin->checkValue($value->coupons[0]->coupon)); 
                                }else{
                                    if( isset($value->_matchingData['Coupons']->coupon) ){
                                        echo h($this->Admin->checkValue($value->_matchingData['Coupons']->coupon)); 
                                    }else{
                                        echo h($this->Admin->checkValue($value->coupon)); 
                                    }
                                }
                            ?>
                        </td>
                        <td data-title="Discount Value">
                            <?php 
                                if($value->discount_type == 'rupees'){
                                    echo $this->Admin->priceLogo.' '.$value->discount_value;
                                }else{
                                    echo $value->discount_value. ' %';
                                } 
                            ?>
                        </td>
                        <td data-title="From Date"><?= h($this->Admin->emptyDate($value->valid_from)); ?></td>
                        <td data-title="To Date"><?= h($this->Admin->emptyDate($value->valid_to)); ?></td>
                        <td data-title="Status"><?= h($this->Admin->checkValue(ucfirst($value->status))) ?></td>
