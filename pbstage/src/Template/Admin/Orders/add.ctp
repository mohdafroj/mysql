...
<script>
	...
	foreach($products as $temp_product)
	{
		...
		productArray[<?php echo $counter;?>]	= {"id":"<?php echo $temp_product->id;?>", "title":"<?php echo $temp_product->title;?>", "sku":"<?php echo $temp_product->sku_code;?>", "price":"<?php echo $temp_product->price;?>"};
		<?php
		$counter++;
	}
	//pr($order);
								...
								<div class="col-sm-12 col-xs-12 flex_box_content price_detail"><!-- start of box_content -->
									<div class="box-body">
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<?= $this->Form->text('shipping_email', ['class'=>'form-control', 'placeholder'=>'Enter pincode', 'id'=>'shipping_email', 'value' => $order->shipping_email]); ?>
											</div>
										</div>
										...
										<?php }else{?>
												<?= $this->Form->text('shipping_state', ['class' => 'form-control', 'id' => 'shipping_state'])?>
										<?php } ?>												
											</div>
										</div>
									</div>
									...
									foreach($order->order_details as $item_row)
									{
										...
										<tr class="order_row order_row_product_<?php echo $counter;?>" data-row="<?php echo $counter;?>"><!-- start of row_2 -->
											<td>
												...
												<input type="text" id="product_<?php echo $counter;?>" name="product_sku[<?php echo $counter;?>]" value="<?php echo $item_row->title;?>" class="form-control product_sku" style="width:250px;" />
											</td>
											<td class="title_product_<?php echo $counter;?>">
												<?php echo $item_row->sku_code;?>
											</td>
											<td class="align_center">
												<input type="number" id="quantity_product_<?php echo $counter;?>" name="product_quantity[<?php echo $counter;?>]" value="<?php echo $item_row->qty;?>" class="form-control align_center cart_change" style="width:75px;" />
											</td>
											<td class="text-center">
												<input type="text" id="price_product_<?php echo $counter;?>" name="product_price[<?php echo $counter;?>]" value="<?php echo number_format($item_row->price, 2, '.', '');?>"  class="form-control cart_change" readonly="readonly" style="width:75px;" />
											</td>
											<td class="text-left subtotal_product_<?php echo $counter;?>">
												<?php echo number_format(($item_row->qty * $item_row->price), 2);?>
											</td>
									...
									<div class="form-group">
										...
										<div class="col-sm-8">
											<select id="payment_method" name="payment_method" class="form-control">
												...
												<option value="2" <?php if($order->payment_method_id == 2) echo 'selected="selected"';?>>PayU</option>
												<option value="3" <?php if($order->payment_method_id == 3) echo 'selected="selected"';?>>PayTM</option>
											</select>
										</div>
									</div>
								</div>
	...
	function manageCoupon(id, reqType){
		...
		if( confirm(msg) ){
			$.ajax({
				type: reqType,
				data: {"id":id},
				url: '<?php echo \Cake\Routing\Router::url(['action' => 'checkCoupon']); ?>',
...
<script type="text/javascript">
		...
		if($('#create_order').length > 0)
		{
			...
			if($( "#customer_name_new" ).length > 0)
			{
				$( "#customer_name_new" ).autocomplete({
					source: "<?php echo $this->Url->build('/admin/customers/search'); ?>",
					minLength: 2,
					select: function( event, ui )
					{
			...
			{
				$('#order_item_table').append('<tr class="order_row order_row_product_'+total_counter+'" data-row="'+total_counter+'"><td><input type="hidden" id="id_product_'+total_counter+'" name="product_id['+total_counter+']" value="" /><input type="text" id="product_'+total_counter+'" name="product_sku['+ total_counter +']" value="" class="form-control product_sku" style="width:250px;" /></td><td class="title_product_'+total_counter+'"></td><td class="align_center"><input type="number" id="quantity_product_'+total_counter+'" name="product_quantity['+total_counter+']" value="1" class="form-control align_center cart_change" style="width:75px;" /></td><td class="align_center"><input type="text" id="price_product_'+total_counter+'" name="product_price['+total_counter+']" value="" class="form-control align_center" readonly="readonly" style="width:75px;" /></td><td class="subtotal_product_'+total_counter+'"></td><td class="align_center delete"><i class="fa fa-trash"></i></td></tr>');
	...
	function getAddresses(){
		...
		if( customerId > 0 ){
			$.ajax({
				url: "<?php echo $this->Url->build('/admin/customers/get-addresses?customer-id='); ?>"+customerId,
				method: 'GET',
				data:[],
