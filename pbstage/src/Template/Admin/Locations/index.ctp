...
<section class="content col-sm-12 col-xs-12">
    ...
    <div class="col-sm-2 col-xs-12 sidebar tree_div"><!-- start of left_part -->
		<?php
			...
			function cateFun($cateTreeItem, $livePath, $data=''){
				//if( count($cateTreeItem->children) > 0 ){
					foreach ($cateTreeItem as $chilName) {
						$child = count($chilName->children);
						if( $child > 0 ){
							$data = $data.'<li class="treeview"><a href="'.$livePath.$chilName->id.'/key/'.md5($chilName->id).'"><i class="fa fa-folder-o"></i> '.$chilName->title.' ('.$child.')<span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a><ul class="treeview-menu">'.cateFun($chilName->children, $livePath, $data).'</ul></li>';
						}else{
		...
		?>
            <ul class="sidebar-menu"><?php echo cateFun($cateTree, $path); ?></ul>
                            ...
                            <div class="col-sm-12 col-xs-12 flex_box_content price_detail"><!-- start of box_content -->
                                <div class="box-body">
									<div class="form-group">
										<label class="col-sm-2 control-label">Parent Location <span class="text-red">*</span></label>
										<div class="col-sm-10">
                                        	<?= $this->Form->select('Locations.parent_id', $cateList, ['empty'=>false,'style'=>'width:100%;','class'=>'form-control'])?>
										</div>
									</div>
										...
										<div class="col-sm-10">
											...
											<span class="text-red">
												<?php
													...
													echo isset($error['code']['code']) ? $error['code']['code']:NULL;
