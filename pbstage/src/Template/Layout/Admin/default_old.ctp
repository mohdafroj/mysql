...
<script>
		...
		if($('#create_order').length > 0)
		{
			...
			if($( "#customer_name" ).length > 0)
			{
				$( "#customer_name" ).autocomplete({
					...
					source: "https://www.perfumebooth.com/pb/api/customers/search",
					minLength: 2,
					select: function( event, ui )
					{
			...
			$('#create_order_btn').click(function(event)
			{
				...
				else if($('#shipping_state').val() == '' || $('#shipping_state').val() == '0')
				{
				...
				else if(same_address == 0 && $('#billing_firstname').val() == '')
				{
					alert('Please enter Billing First Name.');
				}
				else if(same_address == 0 && $('#billing_lastname').val() == '')
				{
					alert('Please enter Billing Last Name.');
				}
				else if(same_address == 0 && $('#billing_address').val() == '')
				{
					alert('Please enter Billing Address.');
				}
				else if(same_address == 0 && $('#billing_city').val() == '')
				{
					alert('Please enter Billing City.');
				}
				else if(same_address == 0 && $('#billing_pincode').val() == '')
				{
					alert('Please enter Billing Pincode.');
				}
				else if(same_address == 0 && ($('#billing_state').val() == '' || $('#billing_state').val() == '0'))
				{
					alert('Please select Billing State.');
				}
				else if(same_address == 0 && $('#billing_phone').val() == '')
				{
					alert('Please enter Billing Phone.');
				}
				else if(same_address == 0 && $('#billing_email').val() == '')
				{
	...
	function setCustomerDetail(customer)
	{
		...
		$('#shipping_firstname').val(customer.firstname);
		$('#shipping_lastname').val(customer.lastname);
		$('#shipping_address').val(customer.address);
		$('#shipping_city').val(customer.city);
		$('#shipping_pincode').val(customer.pincode);
		$('#shipping_phone').val(customer.mobile);
		$('#shipping_email').val(customer.email);
		$('#shipping_state').val(customer.location_id);
		
		
		$('#billing_firstname').val(customer.firstname);
		$('#billing_lastname').val(customer.lastname);
		$('#billing_address').val(customer.address);
		$('#billing_city').val(customer.city);
		$('#billing_pincode').val(customer.pincode);
		$('#billing_phone').val(customer.mobile);
		$('#billing_email').val(customer.email);
		$('#billing_state').val(customer.location_id);
	}
	
	function setCustomerDetailOld()
	{
		var selected	= -1;
		getValue		= $('#customer_name').val();
		for(var i in customerArray)
		{
			if(customerArray[i] == getValue)
			{
				selected	= i;
				break;
			}
		...
		if(selected > -1)
		{
			$('#id_customer_new').val(customerDetailArray[selected].id);
			$('#customer_id span').html(customerDetailArray[selected].id);
			$('#customer_email span').html(customerDetailArray[selected].email);
			$('#customer_name span').html(customerDetailArray[selected].firstname + " " + customerDetailArray[selected].lastname);
			$('#customer_mobile span').html(customerDetailArray[selected].mobile);
			
			$('#shipping_firstname').val(customerDetailArray[selected].firstname);
			$('#shipping_lastname').val(customerDetailArray[selected].lastname);
			$('#shipping_address').val(customerDetailArray[selected].address);
			$('#shipping_city').val(customerDetailArray[selected].city);
			$('#shipping_pincode').val(customerDetailArray[selected].pincode);
			$('#shipping_phone').val(customerDetailArray[selected].mobile);
			$('#shipping_email').val(customerDetailArray[selected].email);
			$('#shipping_state').val(customerDetailArray[selected].location_id);
			
			
			$('#billing_firstname').val(customerDetailArray[selected].firstname);
			$('#billing_lastname').val(customerDetailArray[selected].lastname);
			$('#billing_address').val(customerDetailArray[selected].address);
			$('#billing_city').val(customerDetailArray[selected].city);
			$('#billing_pincode').val(customerDetailArray[selected].pincode);
			$('#billing_phone').val(customerDetailArray[selected].mobile);
			$('#billing_email').val(customerDetailArray[selected].email);
			$('#billing_state').val(customerDetailArray[selected].location_id);
		}
		else
		{
			$('#id_customer_new').val(0);
			$('#customer_id span').html();
			$('#customer_email span').html();
		}
	}
</script>
</body>
