...
class StoreComponent extends Component
{
	...
	public function isPerfumeBooth(){
		$status = 0;
    ...
    public function getWebMessage ($page = '') {
		$message = '15% Supercash on payment through Mobikwik Wallet by using KWIK15';
		$lockdowns = '<p style="padding:0px;margin-bottom:0px;"><b>Please note</b> - </p><p style="padding:0px;">Due to lockdowns, We will ship all orders after 14th April 2020. We apologise for the inconvenience caused.
		In the meanwhile, Stay home, Stay safe!</p>';
		switch($page){
			case 'cart' : $message = '15% Supercash on payment through Mobikwik Wallet'; break;
			default: //$message = '&#x1f3c3; Flat Rs 500 Off on ScentShot || Limited Period Offer';
		}
		$message = '';
		return ['message'=>$message, 'lockdowns'=>'', 'paypal'=>0, 'homeBanner'=>PB_BANNER_STATUS];
    }
	...
	{
		return TableRegistry::get('Products')
		        ->find('all', ['conditions'=>['is_stock' => 'in_stock', 'is_active'=>'active']])
	...
	{
		$result = TableRegistry::get('ZoneCodes')
		        //->find('all', ['fields'=>['id', 'zone_type'], 'conditions'=>["find_in_set($pincode, codes)"]])
		        ->find('all', ['fields'=>['id', 'zone_type'], 'conditions'=>["concat(',', codes, ',') REGEXP ',($pincode),'"]])
				->hydrate(0)
				->toArray();
		return $result[0]['id'] ?? 1; //default is red zone
    }
		...
		foreach($cartData as $temp_cart)
		{
			...
			{   
				if($tempProduct['is_stock'] == 'in_stock' && $tempProduct['is_active'] == 'active' && $tempProduct['qty'] >= $temp_cart['qty'] && $temp_cart['qty'] > 0)
				{
					...
					$cart_data[$counter]['cart_max_qty'] = ( $tempProduct['qty'] > $tempProduct['max_cart_qty'] ) ? $tempProduct['max_cart_qty'] : $tempProduct['qty'];
					...
					$cart_data[$counter]['brand']		= []; //pr($tempProduct['brand']);

					if( !empty($tempProduct['brand']) ){
		...
		if( ($userId > 0) && ($itemId > 0) && ($qty > 0) ){
			...
			if(count($query) > 0){
				...
				if( $checkRefillStatus ){
					...
					$cart->qty = $qty;
					if( $cartTable->save($cart) ){
						//$this->updateStockAfterOrderPlaced([['productId'=>$itemId, 'qty'=>$qty]]);
						$status = 1;
						$message = 'One item added into cart!';
					}else{
						$message = 'Sorry, try aogain!';
					}
				}else{
	...
	public function emptyCart($userId)
	{
		$status = false;
		try{
			$cart = TableRegistry::get('Carts')->query()->delete()->where(['customer_id'=>$userId])->execute();
	...
	{
		$default = 0;
		$paymentData = $this->getActivePaymentGatewayData();
		foreach( $paymentData as $value )
		{
			if( $value['default'] == 1 ){
				$default = $value['id']; break;
			}
		}
		return $default;
	}
	...
	{
		return TableRegistry::get('PaymentMethods')->find('all', ['fields'=>['id', 'title', 'code', 'fees', 'message', 'default'=>'active_default'],'conditions' => ['status' => '1']])->order(['sort_order'=>'ASC'])->hydrate(0)->toArray();
	}
	...
	public function getActiveCartDetails($inputData)
	{
		...
		foreach($cart_data['cart'] as $p ){ 
			$products[] = ['id'=>$p['id'],'qty'=>$p['cart_qty'],'price'=>$p['price'],'category'=>$p['category'][0]['id'] ?? 0,'brand'=>$p['brand']['id'] ?? 0 ];
		}
		...
		if($total_amount_of_cart > 0)
		{
			...
			if ( (PB_BANNER_STATUS != 1) && $customer_voucher_100 && $cart_data['has_refill_product'] ) {
			...
			if( (PB_BANNER_STATUS != 1) &&  empty($couponCode) && $giftVoucherStatus ) { //(PB_BANNER_STATUS != 1)
			...
			if( (PB_BANNER_STATUS != 1) && empty($couponCode) && (($giftVoucherStatus && ($discount_voucher_qty_501 == 0) ) || ($pbPointsStatus && ($pb_points_amount > 0) && ($discount_voucher_qty_501 == 0))) )
			{
			...
			if( (PB_BANNER_STATUS != 1) && $pbCashStatus && $pb_cash_amount > 0)
			{
			...
			$credit_voucher_501		= 0;//$cart_data['credit_voucher_quantity'];
			$credit_voucher_100		= 0;//$cart_data['credit_voucher_100'];
			...
			$allPaymentMethods		= $this->getActivePaymentGatewayData();
			if( $payment_method == 0 ){
				$payment_method = $allPaymentMethods[0]['id'] ?? 0;
			}
			...
			//if total paid amount greater then 5000 then remove cod option
			if( $data['grand_final_total'] > 5000 ){
				$temp = $allPaymentMethods; $allPaymentMethods = [];
				foreach ( $temp as $m ){
					if( $m['id'] != 1 ){
						array_push($allPaymentMethods, $m);
					}
				}
				if( $payment_method == 1 ){
					$payment_method = $allPaymentMethods[0]['id'] ?? 0;
			...
			$data['payment_method_data']			= $allPaymentMethods;
			...
			$data['payment_offer'] = $this->getWebMessage('cart');
		}
		return $data;
	}
	...
	public function logPBWallet($id_customer, $transaction_type, $id_referrered_customer = 0, $id_order = 0, $pb_cash = 0, $pb_points = 0, $voucher_amount = 0, $comments = '', $transaction_ip = 0)
	{
		if($pb_cash > 0 || $pb_points > 0 || $voucher_amount > 0)
		{
			if($id_customer > 0)
			{
				...
				$wallet_log->transaction_ip			= ($transaction_ip == 0) ? $_SERVER['REMOTE_ADDR'] : $transaction_ip;
	...
	public function createInvoiceOld($id_order)
	{
		//try{
			...
			$invoiceOrder					= $invoiceTable->find('all', ['fields'=>['id'], 'conditions'=>['order_number'=>$id_order], 'limit'=>1])->toArray();
			//pr($invoiceOrder);			
			if( empty($invoiceOrder) ){
				...
				if( ($invoice_id > 0) && $invoiceTable->save($invoice) )
				{
	...
	public function sendOrderToDelhivery($id_order)
	{
		...
		$order->delhivery_pickup_id	= 3; //pickup id
