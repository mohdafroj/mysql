        ...
        if (empty($query)) {
            ...
            if (($query->status == 'active') && ($query->_matchingData['Coupons']->status == 'active')) {
                ...
                if (($crtDate > $toDate) && $toDate) {
                    ...
                    //$products = [['id'=>2,'price'=>200,'qty'=>2,'category'=>5,'brand'=>4],['id'=>4,'price'=>200,'qty'=>3,'category'=>5,'brand'=>4],['id'=>11,'price'=>200,'qty'=>1,'category'=>5,'brand'=>4]];
                    $ids = [];
                    $qty = 0;
                    ...
                    if (count($cd['categories'])) {
                        ...
                        foreach ($cd['categories'] as $category) {
                            ...
                            for ($i = 0; $i < count($products); $i++) {
                                //check discounted product
                                 if( ($categoryId == $products[$i]['category']) && ( ($brandId == $products[$i]['brand']) || ($brandId == 0) ) ){
                        ...
                        /*$query = TableRegistry::get('ProductsCategories')->find('all', ['fields' => ['product_id', 'category_id'], 'conditions' => ['product_id IN' => $ids]])->toArray();
                        if (!empty($query)) {
                            $usedStatus = 1;
                            foreach ($query as $v) {
                ...
                if ($rulesData['status']) {                    
                    foreach ($products as $value) {
                        if (in_array($value['id'], $discountProducts)) {
                            $type = $value['type'] ?? $discountType;
                            $discount = $value['discount'] ?? $discountValue;
                            if ($type == 'rupees') {
                                $couponDiscount +=  $discount;
                            }else{
                                $couponDiscount += (float) ($value['price'] * $value['qty'] * $discount) / 100;
                            }
                        }
                    }
            ...
            if (empty($query)) {
                ...
                if (($query->status == 'active') && ($query->_matchingData['Coupons']->status == 'active')) {
                    ...
                    if (($crtDate > $toDate) && $toDate) {
                        ...
                        if (count($cd['categories'])) {
                            $query = TableRegistry::get('ProductsCategories')->find('all', ['fields' => ['product_id', 'category_id'], 'conditions' => ['product_id IN' => $ids]])->distinct(['category_id'])->toArray();
                            if (!empty($query)) {
                                $usedStatus = 1;
                                foreach ($query as $v) {
                                    if (in_array($v->category_id, $cd['categories'])) {
